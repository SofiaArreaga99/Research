...
minDBH<-0.254 #From Jenkin´s
maxDBH<-6.4516 #From Jenkin´s
B0<- -1.7624 #From Ry´s paper
B1<- 2.307  #From Ry´s paper
CF<- 1.004 #Should we include it?
...
dbhFraxisAm <- minDBH + (maxDBH - minDBH) * runif(10000, min = 0, max = 1)
...
dbh2 <- matrix(rep(dbhFraxisAm, times = 1000), nrow = length(dbhFraxisAm), ncol = 1000)
...
BMFraxisAm<- psuedys[, I]  # Select corresponding column

## Create figure for checking if result is reasonable ##


plot(dbhFraxisAm, BMFraxisAm, pch = 16, xlab = "DBH (cm)", ylab = "Biomass (kg)", main = "FraxisAm")
...
plot(PseudoDataFraxisAm$dbhFraxisAm, PseudoDataFraxisAm$BMFraxisAm, pch = 16, xlab = "DBH (cm)", ylab = "Biomass (kg)", main = "FraxisAm")
